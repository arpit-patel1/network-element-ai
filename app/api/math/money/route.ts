...
const moneyCountResponseSchema = z.object({
  problem: z.string().min(1, "Problem is required"),
  items: z.array(z.string()).min(1, "Items are required"),
  answer: z.string().min(1, "Answer is required"),
  answer_cents: z.number().int().positive(),
});

const moneyCalcResponseSchema = z.object({
  problem: z.string().min(1, "Problem is required"),
  answer: z.string().min(1, "Answer is required"),
});

const moneyResponseSchema = z.union([moneyCountResponseSchema, moneyCalcResponseSchema]);

const webhookResponseSchema = z.union([
  moneyResponseSchema,
  z.object({ data: moneyResponseSchema }),
]);
...
export async function GET(request: NextRequest) {
  ...
  const operation = searchParams.get("operation") || "count";
  ...
  try {
    ...
    let url = `${cleanBaseUrl}/math/money?operation=${encodeURIComponent(operation)}`;
